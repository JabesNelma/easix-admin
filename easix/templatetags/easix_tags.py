"""
Easix Template Tags and Filters
"""
...
def icon(name):
    ...
    icons = {
        ...
        "user-plus": '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"></path></svg>',
        ...
        "chevron-up": '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"></path></svg>',
        ...
        "chevron-right": '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>',
        ...
        "x-mark": '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>',
        
        # Dashboard widgets
        "chart-bar": '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>',
        "currency-dollar": '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>',
        ...
        "activity": '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path></svg>',
        "information-circle": '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>',
        ...
        # Status
        "exclamation": '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>',
        "check-badge": '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z"></path></svg>',
    }
    return mark_safe(icons.get(name, icons["information-circle"]))


@register.filter
def badge_class(value, badge_map=None):
    ...
    return badge_map.get(key, badge_map.get(str(value), "gray"))
...
def format_value(value, format_type=None):
    ...
    if format_type == "datetime":
        return value.strftime("%b %d, %Y %H:%M") if hasattr(value, "strftime") else str(value)
...
@register.filter
def truncate_words(value, num=10):
...
@register.simple_tag
def easix_version():
    """Return Easix version."""
    from .. import __version__
    return __version__


@register.simple_tag(takes_context=True)
def is_active_url(context, pattern):
...
@register.simple_tag
def render_icon(name, size="w-5 h-5", class_name=""):
    """Render an icon with custom size and class."""
    icons = {
    ...
    path = icons.get(name, icons["settings"])
    return format_html(
        '<svg class="{} {}" fill="none" stroke="currentColor" viewBox="0 0 24 24">{}</svg>',
        size,
        class_name,
        mark_safe(path)
    )
...
@register.simple_tag
def widget_class(width):
    """Get widget grid class based on width."""
    classes = {
        "full": "col-span-12",
    ...
    return classes.get(width, "col-span-12")


@register.simple_tag
def field_widget_class(field_type):
    """Get widget class for field type."""
    classes = {
        "text": "form-input",
        "email": "form-input",
        "password": "form-input",
        "number": "form-input",
        "textarea": "form-input min-h-[100px]",
        "select": "form-input",
        "checkbox": "rounded border-gray-300 text-indigo-600 focus:ring-indigo-500",
        "radio": "border-gray-300 text-indigo-600 focus:ring-indigo-500",
        "date": "form-input",
        "datetime": "form-input",
        "file": "form-input",
        "image": "form-input",
    }
    return classes.get(field_type, "form-input")


@register.inclusion_tag("easix/components/badge.html", takes_context=True)
def badge_tag(context, value, style="default"):
    """Render a badge."""
    styles = {
    ...
    # Auto-detect style from value
    if style == "auto":
        if value is True or str(value).lower() in ["active", "published", "approved", "yes"]:
            style = "success"
        elif value is False or str(value).lower() in ["inactive", "draft", "rejected", "no"]:
            style = "danger"
        elif str(value).lower() in ["pending", "processing"]:
            style = "warning"
        else:
            style = "default"
    ...
    return {
        "value": value,
        "class": styles.get(style, styles["default"]),
    }


@register.inclusion_tag("easix/components/avatar.html", takes_context=True)
def avatar_tag(context, user, size="md"):
    """Render user avatar."""
    sizes = {
        "sm": "w-6 h-6 text-xs",
        "md": "w-8 h-8 text-sm",
